...
const App = () => {
  ...
  const fetchOrganizerData = async (email) => {
    try {
      ...
      if (!organizerRes.ok) {
        console.error("Organizer fetch failed:", organizerData.message);
        setLoading(false);
        return;
      }
      ...
      setClubData(dashboardData.managingEntity); // could be club/society/faculty
      setEvents(dashboardData.managingEntity?.events || []);
      setLoading(false);
    } catch (error) {
